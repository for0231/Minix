 ...
 */
  ...
  if (machine.protected) {
       ...
       */
      outb(INT_CTL, machine.ps_mca ? ICW1_PS : ICW1_AT);  //#define INT_CTL         0x20	/* I/O port for interrupt controller *
...
irq_hook_t *hook;   //typedef int irq_hook_t;
int irq;
irq_handler_t handler;//typedef int (*irq_handler_t)(struct irq_hook *);
{
/* Register an interrupt handler. */
  ...
  line = &irq_handlers[irq];        //EXTERN irq_hook_t *irq_handlers[NR_IRQ_VECTORS];/* list of IRQ handlers */
  id = 1;
  while (*line != NULL) {
 ...
 */
  ...
  while (hook != NULL) {
       ...
       */
      irq_actids[hook->irq] |= hook->id;  //EXTERN int irq_actids[NR_IRQ_VECTORS];		/* IRQ ID bits active */
